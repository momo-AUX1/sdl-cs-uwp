...
namespace xbox_uwp_sdl2_starter
{
    public static class GL
    {
        ...
        {
            IntPtr proc = SDL_GL_GetProcAddress(name);
            if (proc == IntPtr.Zero)
            {
                Debug.WriteLine($"Unable to load OpenGL function '{name}'. Ensure that the function name is correct and that the OpenGL context has been created.");
                throw new Exception($"Unable to load OpenGL function '{name}'. Ensure that the function name is correct and that the OpenGL context has been created.");
            }
